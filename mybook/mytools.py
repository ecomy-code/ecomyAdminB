...
def insertar_producto_nuevo(id, idemp, name, detalle, precio, fotoa, fotob):
    try:
        connection = db.connect(
            host='localhost', database='productos', user='root', password='azby')
        cursor = connection.cursor()
        queryx = """ INSERT INTO productos (id,idemp, name, detalle, precio, fotoa, fotob) VALUES (%s,%s,%s,%s,%s,%s,%s)"""
        fotoak = convercion_aBinaryText(fotoa)
        fotobk = convercion_aBinaryText(fotob)
        myproductdates = (id, idemp, name, detalle, precio, fotoak, fotobk)
        result = cursor.execute(queryx, myproductdates)
        connection.commit()
        connection.close()
        print("Guardaste con exito tu producto jefe", str(result))
...
# try:
 #   insertar_producto_nuevo(2,"ecomycr","@ecomy-admin", "Mi primer aplicación al mercado.", "1.000.000.000.000.000,89", "src/asd.png", "src/asd.png", "19 de octubre")
